...
**Política de testes:** Playwright E2E + testes unit/integration conforme CONSTITUTION; quando existir, alinhar a `.specs/codebase/TESTING.md`
...
<!-- Código de aplicação neste projeto: -->

- **App full-stack:** `apps/web/` (Next.js App Router, Server Actions/Route Handlers, SQLite)
- Paths típicos: `apps/web/src/`, `apps/web/tests/` ou equivalente definido no plano da feature
...
- [ ] T005 [P] [INFRA] Framework de auth / ownership em `[path de auth]`
...
| **T005** [P] | INFRA | [Auth / ownership] | `[path]` | T004 | FR-002 | unit | `[cmd test]` |
...
- Commit após cada task: `feat(MN-slug): descrição da task` (no repositório de código `apps/web`)
- Pare em qualquer checkpoint para validar de forma independente antes de continuar
- Respeitar glossário: Despesa, Categoria, Arquivar — não usar Gasto/Lançamento/Transação como sinônimo de registro
